...
var mod = angular.module('iso-3166-country-codes', []);
mod.factory('ISO3166', function() {
	var holder = {};
	...
	holder.getCountrysByArray = function (countryCodes) {
		...
		angular.forEach(countryCodes, function(key){
			if (angular.isDefined(holder.codeToCountry[key])) {
				countrys[key] = holder.codeToCountry[key];
			}
		});
		return countrys;
	};
...
mod.filter('isoCountry', ['ISO3166', function(ISO3166){
	return function (input) {
		var result = ISO3166.getCountryName(input);
		return angular.isUndefined(result) ? input : result;
	};
}]);
mod.directive('countryCode', ['ISO3166', function(ISO3166) {
	return {
		require: 'ngModel',
